 ...
 * @brief   点阵屏 UI 框架 —— 平台配置总入口
 *
 * 本文件承担平台 SDK 里 app_config.h + board_config.h 两者的角色。
...
/* ========================================================================
 * 一、功能裁剪开关(对应 SDK 侧 app_config.h 的 TCFG_*)
 * ======================================================================== */
 ...
 * 且既有配置就是单 buf(BUF_NUM=1), 保持一致以免行为漂移 */
 ...
 * SDK 侧是在编译命令行里定义它, 本工程放在配置头里更直观。 */
#define EXPORT_DOT_UI_ENABLE    1

/* CONFIG_CPU_BR27 用来选 IMD 硬件推屏。STM32 无 IMD, 一律【不要】定义
...
/* 框架递归重绘控件树 + 字模取模, 栈需求偏大。参考配置给的是 1024 字(4KB),
...
/**
 * 歌词时间标签【直存 flash】。本工程不需要, 固定为 0。
 *
 * 这是为"长歌词文件"做的优化: 把解析好的时间标签索引写回
 * norflash, 下次直接读索引而不用重新解析整个文件。它依赖三个
 * 前提, 本工程一个都不成立:
 *
 *   1) 要能拿到文件在 flash 上的物理地址。SDK 侧靠 vfs 的起始簇号
 *      (resfile_attrs.sclust)换算; FATFS 不对外暴露簇号, 本工程恒填 0。
 *   2) 要能直接擦写那块 flash。本工程资源在 FATFS 卷上, 绕过文件系统
 *      去擦扇区会直接写坏卷。
 *   3) 要能把 flash 地址当指针解引用(XIP 内存映射)。目标平台的 norflash
 *      挂在可寻址总线上; 本工程的资源盘拿不到线性地址。
 *
