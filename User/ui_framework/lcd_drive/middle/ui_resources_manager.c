...
/* 资源加载失败是上板排查的头号问题(界面全黑基本都是它), 错误日志必须留。
...
void *jlui_load_widget_info(void *__head, u8 page)
{
    ...
    if (page != (u8) - 1) {
        ...
        if (load_style && platform_api->load_style) {
             ...
             * 保持未初始化, 于是:
             *   1) rotate[head.rotate] 越界读(rotate[] 只有 4 项);
             *   2) window_offset 也是垃圾, 后续按垃圾长度 malloc,
             *      表现为 "Memory allocation failed!" + 断言停机, 完全看不出
             *      真实原因是"资源没放进文件系统"。
             * 这里如实向上报错, 让 window_show 干净失败。 */
            int style_err = platform_api->load_style(&style);
            load_style = 0;
            if (style_err) {
                log_error("load_style failed(%d): resource missing? "
                          "check %s exists on the filesystem\n",
                          style_err, RES_PATH "JL/JL.sty");
                return NULL;
            }
        }
        ...
        if (res_fread(ui_file, &__this->window_offset, sizeof(__this->window_offset))
            ...
            log_error("read window_offset failed, page %d\n", page);
            return NULL;
        }
    }
