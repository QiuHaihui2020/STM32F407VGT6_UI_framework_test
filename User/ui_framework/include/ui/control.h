 ...
 *
 * 表本体在 config/ui_port_registry.c。新增控件时必须往那张表里加一行。
 * ==================================================================== */

/** 全部已注册控件的 ops 指针表, 以 NULL 结尾 */
extern const struct control_ops *const g_control_ops_table[];

/**
 * @brief 按控件类型取操作集
 * @param type CTRL_TYPE_* 之一
 * @return 找到返回 ops; 找不到返回 NULL(调用方必须判空)
 */
const struct control_ops *get_control_ops_by_type(int type);

/** 定义一个控件的 ops。注意【不是 static】—— 注册表要跨文件引用它 */
#define REGISTER_CONTROL_OPS(_type) \
    const struct control_ops control_ops_##_type __attribute__((used)) = { \
        .type = _type,
