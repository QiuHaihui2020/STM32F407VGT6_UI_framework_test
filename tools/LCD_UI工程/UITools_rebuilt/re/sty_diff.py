...
用法: python sty_diff.py <本版.sty> <原厂.sty> [ename.h]
"""
import struct
import sys
...
def main():
    ...
    for pi, (wa, wb) in enumerate(zip(A['windows'], B['windows'])):
        ...
        if da != db:
            ...
            print('   本版 %s' % da[max(0, i - 8):i + 24].hex())
            print('   原厂 %s' % db[max(0, i - 8):i + 24].hex())
            ndiff += 1
        # 重定位表
        ...
        if ta != tb:
            ...
            print('页%d 重定位表 %d/%d 项，首个不同 @%d: 本版 0x%04X 原厂 0x%04X'
