# QtToolBin / ResBuilder 的实现与验收

整链三个工具的产物和既有资源的差距：

| 工具 | 与既有资源的差距 |
|---|---|
| `UITools.exe`    | 工程 json 读写**逐字节相同** |
| `QtToolBin.exe`  | `project.bin` 24498 B 里差 **61 B**，全是参考文件里的未初始化内存 |
| `ResBuilder.exe` | `result.str` 差 **4 B**（resver）；`result.bin` 差 4 B + 调色板顺序 |
...
**那些"差异都可解释"分别是什么**——都不是能修的 bug，是那几格本来就不可复现：

| 差异 | 字节 | 原因 |
|---|---|---|
| `project.bin` | 61 | 8 个 Time/number 控件的 `char format[16]` 尾部，参考文件里是没清零的堆内存，各控件互不相同（见 FILE_FORMATS 9.4）。重跑一次也会变 |
| `result.bin` / `result.str` | 各 4 | 文件头的 `resver`。参考文件里是随机值——空资源文件里也非零。这里改成内容 CRC32 |
| `result.bin` / `result.xml` | 75 / 146 | 调色板前缀的**排列顺序**。集合一致，顺序不同（不是任何一种树遍历）。OSD1 单色屏不读调色板 |
| `res_ver.h` / `result_*_index.h` | 21 / 5 / 5 | 生成时间戳 |

`--ename <现成的 ename.h>` 用来复用已分配的 id，只在做上面这种逐字节比对时加；
正常使用不要加，会自己分配（见第 3 节）。
...
| 字符串 | UITools | QtToolBin | ResBuilder |
|---|:--:|:--:|:--:|
| `uitoolbin` | – | ✔ | – |
| `ename.h` | – | ✔ | – |
| `project.bin` | – | ✔ | – |
...
验证脚本 `compat/verify_types.py`，在真实工程上的结果：
...
id = (pj_id << 29) | (page << 22) | (type << 16) | low16
     pj_id ≤ 7     page ≤ 0x7f    type ≤ 0x3f
```

`pj_id` 就是界面上那个"工程ID"，`page` 是页序号，`type` 是控件类型码
（和 `.sty` 控件头里的 type 同一个值）。

### 低 16 位：既有资源里是一份查表，离线复现不了

既有 `ename.h` 里那 281 条的低 16 位试遍了常见哈希都对不上
（281 条样本全部 0 命中）：
...
- 也确认了这些值不在工程 json、uitoolbin.bin、Resbuilder.dat、result.* 等
  任何中间文件里
...
所以这里用**自己的确定性低 16 位**（`CRC16(ename)` + 冲突时线性探测），
产出的 `.sty` + `ename.h` 完全可用，只是不会和既有那份**逐字节**相同——
而那份本来就不可复现。
...
- 拆开重拼 24498 字节逐字节一致（`compat/sty_roundtrip.py`）
...
- 数据区的**排布算法**（组间倒序、组内按字段偏移升序、4 字节对齐 0xFF 填充、
  空列表也占位）—— `compat/sty_gen.py` 丢掉全部偏移后重新排布，**逐字节相同**
- 重定位表的生成规则 —— 3/3 页重建一致
- 页表里三个 CRC 的定义（`crc_head` / `crc_data` / `crc_table`，均为 CRC16-XMODEM）
  —— 9/9 验证通过，生成器已自算

**已全部补齐**：窗口记录 28 字节的字段含义（`compat/verify_window.py` 18/18）、
css 字段映射与万分比换算（`compat/verify_css.py` 273/273）、控件负载字段
（`compat/verify_payload.py`）、图片/字符串资源号（`compat/verify_resids.py` 511/511）、
记录排列顺序（`compat/verify_order.py` 3/3 页）。生成器 `src/sty/StyBuilder.cpp`
直接吃工程 json 产出 `project.bin`，与既有资源只差那 61 字节未初始化内存。
...
3. **万分比是向上取整**。`95*10000/128 = 7421.875 -> 7422`。
   注意 C++ 整数除法向零截断，不能直接套 Python 的 `-((-a)//b)`，
   要写成 `(num + ref - 1) / ref`。
...
10. **`prop_len` = `24 + 20*页数 - 16`**（3 页 = 68）。固件不读它，但比对要一致。

## 8. 还没做的

| 项 | 情况 |
|---|---|
| `action` 块的**内容** | 结构已知（固件 `struct event_action`：`u16 event; u16 action; int id; u8 argc; char argv[]`，对齐 4），生成器也照着写了，但本工程 277 个控件全是 `num=0`，**没有真实样本可比对** |
| 调色板前缀顺序 | 集合已复现，顺序没有。单色屏无影响 |
| `debug.txt` 排版 | 内容对（同一份 .sty 的十六进制转储 + 页表摘要），空白/日期格式与参考文件略有出入。它只是调试辅助文件 |

---

## 「功能设置」（又叫「配置界面」）

这一页的每一项最终都落进 `Resbuilder.xml`，也就是 ResBuilder 的唯一输入。

### 验过的（有现成文件做依据）

| 结论 | 依据 |
|---|---|
| 字段名/格式：`language` 是 8 位小写十六进制、`bmp_transparent_color` 是大写 | 工具目录里那份遗留的 `Resbuilder.xml`（240x240 twsbox） |
| 「选中: N」= `<language>` 掩码，选中的行就是置位的位 | 两个独立来源：界面上 `选中: 13` 且 1/2/5 行高亮（0x13 = bit0/1/4）；现成 xml 里 `0x00000007` 对应简中+繁中+日语 |
| 「字号」是磅值，`lfHeight = -(字号 x 4 / 3)` | 界面上前 6 行 24、第 7 行 12；现成 xml font00–05 是 -32、font06 起 -16 |
| 「资源文件名」带出 `res` / `resfilename` / `headerfilename` 三项 | 现成 xml：`result` / `result.bin` / `result.h` |
| 默认值改成现成那张字体表后产物不变 | `result.str` 与改动前**逐字节相同**（0 差异），整链验收其余项判定不变 |
...
| 压缩方式除 `none` 外的取值名 | 列了 `rle` / `quicklz` | 这里的 ResBuilder 只实现 none，选了也不压；下拉框上有提示 |
| 设置存哪 | 工程目录的 `Resbuilder.xml` | 另有一份 `Resbuilder.dat`（941KB，`"1.2"` 开头 + 一串 LOGFONT 记录）是逐格字体缓存，格式见 FILE_FORMATS 10.6 |

要把这几项坐实，得在界面上逐项改、每改一次看 `Resbuilder.xml` 怎么变。
那是交互操作，脚本代劳不了。

### 定位差异出在谁身上（2026-09-09）

同一个 ResBuilder，分别喂两份 `Resbuilder.xml`：

| 输入 | 结果 |
|---|---|
| **本次生成**的 Resbuilder.xml | result.str / result.h / result.csv **逐字节相同**；result.bin 差 79 字节 |
| **现成那份** Resbuilder.xml（对照组） | result.bin **0 差异** |

同一个 ResBuilder，喂现成 xml 得 0、喂本次 xml 得 79 —— 差异 **100% 来自
生成的 Resbuilder.xml**，与 ResBuilder 本身无关。

再定位那 79 字节：4 个在 0xC..0xF（resver，本来就是随机值），
75 个从 0x374 起 —— 全在**各页调色板**那一段。也就是说：

**除了 resver 和 ColorList 的排列顺序，产物与既有资源逐字节相同。**
（OSD1 单色屏不用调色板，所以这条对本项目没有功能影响；但要做到 100%
字节替换，得把 ColorList 的排序规律挖出来。）

#### 这一轮改对的四处（都是对照现成文件查出来的）

| 问题 | 原来 | 应该是 | 怎么发现的 |
|---|---|---|---|
| 编码 | `toLocal8Bit()`（本机 GBK） | **UTF-8** | 现成的 `Resbuilder.xml` 里"多"是 `E5 A4 9A`；本机 ACP 是 936 |
...
```
参考: 0000FF D2EE45 FF557F 368FEE FF0000 AAA555 D9EE94 FFFFFF 000000 FFAA7F 55AA00
本次: 0000FF 55AA00 FF557F FF0000 D9EE94 FFAA7F FFFFFF D2EE45 368FEE AAA555 000000
```

线索：两边都以 `0000FF` 开头、第 3 位都是 `FF557F`；参考里的 `000000` 落在第 9 位
（**不是**补在末尾，这里是"缺了就 append"）。这里的收集顺序是"逐节点：先 css
...
3. 用 ResBuilder 吃它，比 result.bin，目标 0 差异
...
* 拿同一个 ResBuilder 分别吃两份 xml，解出来的调色板
  **与各自 xml 里 ColorList 逐项一一对应** —— ResBuilder **不重排**，原样写出。
* 两边调色板的**集合完全相同**，仅顺序不同（`参考独有: []  本次独有: []`）。

所以：**这 75 字节差异 100% 来自 ColorList 的排序，别无其它来源。**
...
最可能的解释是当初用某种 Qt 关联容器（`QSet`/`QHash`）收集颜色、直接按容器序
输出，而 Qt5 的 `qHash(QString)` 默认带**进程级随机种子** —— 那样的话
重跑一次顺序也会变，根本不存在可复现的"正确顺序"。这和已知的
...
对同一个工程连跑两次，比两次的 ColorList：

* 两次**不同** → 坐实是随机序，本项到此为止，改成"记录已知差异"即可；
* 两次**相同** → 说明有确定性算法，再拿这两份样本继续找规律。

这一步得手工点两次「生成资源文件」，脚本代劳不了。

#### 结案（2026-09-09，连跑两次实测）
...
**结论：不存在"正确的 ColorList 顺序"——它每次启动都不一样。**
本项永久结案，不再尝试对齐。和"控件 id 低 16 位每次生成都不同"是同一类现象。

守住真正重要的那条线即可：`compat/verify_palette_order.py` 检查**集合**一致
...
2026-09-09 再次生成参考之后，`--verify-resxml` 报了「Fonts 不一致（参考 5816
字符 / 本次 5816）」。逐行看下去，**每个属性的值都一样，只是排列不同**：

```
参考: <font00 lfOrientation lfClipPrecision lfWidth lfHeight lfFaceName lfCharSet
             lfUnderline lfOutPrecision lfItalic lfWeight lfQuality lfStrikeOut
             lfPitchAndFamily lfEscapement />
本次: <font00 lfQuality lfPitchAndFamily lfOrientation lfHeight lfWeight lfUnderline
             lfOutPrecision lfItalic lfClipPrecision lfFaceName lfCharSet
             lfEscapement lfStrikeOut lfWidth />
```

这个次序是当初照着**某一次**产物定下来硬编码的。现在参考换成了另一次
进程的产物，次序就对不上了 —— 和 ColorList 一样，LOGFONT 的字段是塞在
...
这样比的是"配置内容一不一致"，而不是"这次碰巧怎么排"。改完 0 处不一致。
