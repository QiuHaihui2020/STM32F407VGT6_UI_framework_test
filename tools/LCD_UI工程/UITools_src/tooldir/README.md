...
点阵屏 UI 工具链的工具目录：三个 exe + 它们要用的配置和数据文件。
工程里按 `..\..\..\UIToolkit\` 调用它，固件那边一行都不用改。
...
| 文件 | 干什么 |
|---|---|
| `config\ini\option.ini` | 控件类型码表 |
| `control\` | 控件库 + `ex\` 扩展模板 |
| `backgrounds\` | 画布背景图 |
| `Application Data\ui-config` | 编辑器默认值 |
| `多国语言_128_64.xls` | 多国语言表 |
| `Resbuilder.xml` | 模板，工程里没有时拷一份过去 |
...
配套的工程目录是 [`../ui_128_64_app/`](../ui_128_64_app/)。
每个 `<界面>\` 下三个脚本，双击即可：
...
| `step3-自检.bat` | 检查产物自洽性（21 项，见下） |

也可以直接对着任意一个同样深度的工程目录跑：

```
cd <工程>\<界面>\project
..\..\..\UIToolkit\QtToolBin.exe --run-resbuilder ..\..\..\UIToolkit\ResBuilder.exe
```
...
`ui_128_64_app\` 一致，模板在本目录的 `template\`），然后：
