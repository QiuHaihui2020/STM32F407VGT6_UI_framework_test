...
`tools/LCD_UI工程/UITools/` 下那三个 exe 的源码从来没随 SDK 交付过。
这里是**从二进制逆向出结构后重写的 Qt 工程**，三个工具全部重建、编译通过、
产物与原厂逐字节对拍：

| 原厂 | 重建版 | 干什么 | 与原厂产物的差距 |
|---|---|---|---|
| `ui-tools.exe`   | `UITools.exe`    | 布局编辑器，读写工程 json | **逐字节相同** |
| `QtToolBin.exe`  | `QtToolBin.exe`  | 工程 json → `project.bin`(=JL.sty) + `ename.h` + `Resbuilder.xml` + `debug.txt` | 24498 B 里差 **61 B**（原厂未初始化内存） |
| `ResBuilder.exe` | `ResBuilder.exe` | `Resbuilder.xml` + bmp + xls → `result.bin`(=JL.res) / `result.str`(=JL.str) + 5 个头文件 | `result.str` 差 **4 B**（随机 resver） |

不是反编译产物。静态链接的 Qt C++ 程序反编译出来是几十万行不可编译的伪代码，
没有工程价值。真正能还原的是**结构**——Qt 把类名、继承链、全部
signals/slots/properties/enums 明文存在 `staticMetaObject` 里，strip 删不掉；
qrc 资源、uic 生成的控件名也都在。二进制格式则靠**固件自己的解析器**
（`User/ui_framework/` 下的 `resfile.c` / `ui_resources_manager.c` / `control.h`）
反过来推写侧，再拿真实工程逐字节对拍。

| | |
|---|---|
| 逆向报告（怎么挖的、挖到什么） | [`docs/RE_REPORT.md`](docs/RE_REPORT.md) |
| 文件格式规范（工程 json / `.sty` / `.res` / `.str`） | [`docs/FILE_FORMATS.md`](docs/FILE_FORMATS.md) |
| QtToolBin / ResBuilder 重建依据与验收 | [`docs/QTTOOLBIN.md`](docs/QTTOOLBIN.md) |
| 原厂界面实测对照（布局/配色/图标/类映射） | [`docs/FACTORY_UI.md`](docs/FACTORY_UI.md) |
| 43 个类的完整接口（逆向原文） | [`docs/RECOVERED_CLASSES.txt`](docs/RECOVERED_CLASSES.txt) |
| 逆向与验证脚本（全部可复跑） | [`re/`](re/) |
...
它会用重建版 `QtToolBin` + `ResBuilder` 从工程 json 重新生成全部产物，
再和工程目录里原厂那份逐字节对比。本机实测：
...
那几处"允许范围内"**不是没做完，是原厂自己也复现不了**：

- `project.bin` 的 61 B —— 8 个 Time/number 控件 `char format[16]` 的尾部，
  原厂没清零，里面是各不相同的堆指针。原厂重跑一次也会变。
- `result.bin` / `result.str` 各 4 B —— 文件头的 `resver`。原厂是随机值
  （一个只有 16 字节头、什么资源都没有的 `result.bin`，`resver` 也非零）。
  重建版改成内容的 CRC32：确定性、可 diff，"内容变了版本号就变"的原意还在。
...
上面那张表是**借了原厂已经分配好的 id**（`--ename`）跑出来的，证明的是
"同样的输入我算出同样的字节"。但真正替换的时候不会借 —— id 由重建版自己分配，
`.sty` 里的 id 和 `ename.h` 里的宏值都得是我们自己的，两边必须严丝合缝。
这一项由另一个脚本管：

```
python re/verify_selfconsistent.py <产物目录> <固件 include/common 目录>
```
...
- **产物是确定性的**。同一份工程连跑两遍，`project.bin` / `ename.h` /
  ...
  这点比原厂**强**：原厂的 id 是随机分配的，同一份工程每次生成都不一样。
  重建版用 `CRC16-XMODEM(宏名)` + 线性探测，只有增删/改名控件时 id 才会变。
- **新 `ename.h` 的宏名集合与原厂完全相同**（282 个，一个不差，只有值不同），
...
`STRING_VERION`）对不上不会有任何后果 —— 这也是"重建版把 resver 改成内容 CRC32"
这个决定完全安全的原因。`UI_VERSION` 同理，但重建版仍然让 `.sty` 头和 `ename.h`
...
`UITools/` 下五个 exe 分工明确（在各自二进制里搜关键字符串确认的）：

| exe | 职责 |
|---|---|
| **`ui-tools.exe`** | **纯布局编辑器**，只读写 `<工程>.json` 和 `autosave.json` |
| `QtToolBin.exe` | `uitoolbin.bin`(同一份 json) → `project.bin`(=JL.sty) + `ename.h` + `debug.txt` + `Resbuilder.xml` + `UI_VERSION` |
| `ResBuilder.exe` | `Resbuilder.xml` + bmp → `result.bin`(=JL.res) / `result.str`(=JL.str) |
| `QTToolJson 1.1.exe` | json 转换（坐标从绝对像素换算成万分比） |
| `UI工程源文件升级工具.exe` | 老版本工程升级 |

`uitoolbin` / `ename.h` / `debug.txt` / `project.bin` / `Resbuilder.xml` /
`UI_VERSION` 这些字符串**只在 `QtToolBin.exe` 里有，`ui-tools.exe` 里一个都没有**。
所以 `ui-tools.exe` 不产资源、不调下游工具，替换它的门槛就是：
**工程文件读写要和它完全一致**。

后两个 exe 是资源生成链：`QtToolBin` 吃工程 json，吐 `.sty` 和
`Resbuilder.xml`，再调 `ResBuilder` 把图片和多国语言表打成 `.res` / `.str`，
最后跑 `copy_file.bat` 拷进固件工程。重建版把这条链完整实现了，
...
- **往返**证明"没改过的东西原样带出去"。工程 json 就是 Qt 的
  `QJsonDocument::toJson(Indented)` 输出（`re/qtjson.py` 用纯 Python 复刻了它的
...
UITools.exe --tools-root <T> --ops-test --out 报告.txt <工程.json>
                                               # 编辑操作与限制，无人值守 20 项
UITools.exe --tools-root <UITools> --shot a.png <工程.json>
```
...
也支持 qmake（`UITools.pro`）和 Qt6。原程序本身是 Qt 5.9.3 / i386 /
mingw 静态；想复刻那个形态就用 32 位 mingw + 静态 Qt 5.9.3。
...
```
UITools.exe --tools-root ..\UITools  ..\ui_128_64_JL02\模式界面\project\SmallColorTFT.json
```
`--tools-root` 指向 `UITools` 目录，控件库 / 图标 / 多国语言表都从那儿读。
...
| `--ename <ename.h>` | **只用于对拍**：复用原厂已分配的 id |
| `--verify <目录>` | 生成后与该目录里的原厂产物逐字节比对 |
...
已经做好了，是**成对的两个目录**，和原厂那一对一一对应：

```
tools\LCD_UI工程\
    UITools\                     原厂工具目录（一个字节没动）
    ui_128_64_JL02\              原厂工程目录（一个字节没动）

    UITools_rebuilt\             ← 对标 UITools\        22 MB
    ui_128_64_JL02_rebuilt\      ← 对标 ui_128_64_JL02\ 15 MB
```

两边的相对路径约定完全一样（工程里都是 `..\..\..\<工具目录>\`），
所以原厂那套用法怎么用，这套就怎么用。

### `UITools_rebuilt\` —— 工具目录

```
UITools.exe  QtToolBin.exe  ResBuilder.exe        三个工具
Qt5*.dll  platforms\  imageformats\  styles\      Qt 运行库，不用配环境变量
config\ini\option.ini  control\  backgrounds\     从原厂原样拷来
Application Data\  多国语言_128_64.xls  Resbuilder.xml
re\                                               验证脚本
template\  新建工程.bat                            拉空工程
README.md                                         与原厂的**逐项**对照
```

原厂 19 个条目逐项交代在 `UITools_rebuilt\README.md` 里：9 项有对应、
**2 个 exe 没做**（`QTToolJson 1.1.exe` / `UI工程源文件升级工具.exe`，没逆向，
正常流程用不到，要用就还用原厂的）、7 项是原厂目录里的缓存和空跑残留（不需要）、
其余是重建版多出来的。

### `ui_128_64_JL02_rebuilt\` —— 工程目录

```
clear.bat  clear.sh  README.md
模式界面\
    step1-打开UI绘图工具.bat        对应原厂同名
    step2-生成资源.bat              对应原厂 step2-打开UI资源生成工具.bat
    step3-自检.bat                  新增
    打开图片资源文件夹.bat           对应原厂同名
    project\
        SmallColorTFT.json  SmallColor_oled.json    从原厂原样拷来
        config\  backgrounds\  Application Data\    同上
        copy_file.bat  release.bat  version.txt
        （生成物：跑 step2 就有）
```

没拷原厂那些缓存和中间文件（`autosave.json` 5 MB、`uitoolbin.bin` 4.6 MB、
`Resbuilder.dat`、`qtread.csv`、`imagelist.txt`），所以 25 MB 变 15 MB。

> `clear.bat` 和原厂那份有一处**故意不同**：原厂会 `del config\*.png`（文字预览图），
> 那是原厂编辑器生成的，重建版**不产**这些图，删了找不回来，所以这里不删。

### 重新生成这两个目录

```
build.bat              编出三个 exe 到 C:\bt\uitools
mkdist_tooldir.bat     装配 ..\UITools_rebuilt\
mkdist_project.bat     装配 ..\ui_128_64_JL02_rebuilt\
```
...
step2 和原厂一样是弹窗的（界面上有进度条和输出）；step1/step3 直接干活。
三个都**不带参数** —— 工具目录按 `..\..\..\UITools_rebuilt` 找，
其余参数从 `project\config\ini\project.ini` 读，**和原厂读的是同一份配置**：
...
新工程：`UITools_rebuilt\新建工程.bat <工程名> [界面名]`。
...
> **id 会变，资源和头文件必须一起换。** 重建版分配的控件 id 和原厂不一样
> （原厂是随机的，同一份工程每次都不同；重建版是 `CRC16(宏名)` + 线性探测，
> **确定性**的，只有增删/改名控件时才变）。`copy_file.bat` 本来就是把
> `ename.h` 和三个资源文件一起拷过去的，照常跑没事；但**不要**手工只拷
> `JL.sty` 而不更新 `style_jl02.h` —— 那样 id 对不上，界面整个不出来。

> **换回原厂**：用原厂那两个目录里的 step1/step2 即可，它们全程没动过。

---

## 界面

按原厂布局复刻，逐项对照见 [`docs/FACTORY_UI.md`](docs/FACTORY_UI.md)：
无菜单栏、无状态栏；一排"图标+文字"工具栏，状态文字在最右；左侧三列树
（结点/属性/ID号 + 眼睛列 + 底部"控件数量"）；第二列「控件列表」组框
（图层/布局大按钮 + 2 列控件网格 + 自定义控件子组）叠属性区（ID号 →
CSS属性_N → 控件专有属性）；右侧页面栏逐页渲染；画布贴左上角。配色采样自原厂
（面板绿 `#C0DCC0`、属性区 `#CEE2CE`、画布 `#F0F0F0`）。

属性面板是**数据驱动**的：`CSS属性_N` 的页签数 = `element_css.struct` 的长度，
每页的内容按该状态里的属性描述（`-type` / `enum` / `default` / `min` / `max`）
现场生成。写死反而会和别的控件对不上。
...
├─ build.bat            编三个 exe
├─ dist.bat             打成能独立运行的一套
├─ mkdist_tooldir.bat   装配 ..\UITools_rebuilt\（对标原厂 UITools\）
├─ mkdist_project.bat   装配 ..\ui_128_64_JL02_rebuilt\（对标原厂工程目录）
...
│    ├─ Forms.h Canvas.h Docks.h Property.h MainWindow.h ProjectDialog.h  手写
│    └─ 其余 15 个   由 re/gen_classes.py 依据逆向元数据生成
├─ src/
│    ├─ core/        ProjectModel（工程 json，保真往返）/ ControlLibrary / StyFile
│    ├─ ui/          Forms / Canvas / Docks / Property / MainWindow / ProjectDialog
│    ├─ gen/         15 个生成的骨架实现（签名对，函数体待补）
│    ├─ main.cpp     编辑器入口
...
├─ resources/        从 exe 里扔出来的 77 个图标 + uitools.qrc
├─ re/               逆向与验证脚本（全部可复跑，见下）
└─ docs/             报告 / 格式规范 / 界面对照 / 接口原文
```
...
| 脚本 | 验的什么 | 本机结果 |
|---|---|---|
| `verify_toolchain.py` | **整链**：重建版产物 vs 原厂产物（借原厂 id） | 不合格 0 项 |
...
| `verify_palette.py` | 调色板构成（含提取内置 255 色表） | 3/3 页 |
| `moc_scan.py` / `qrc3.py` / `strlit_scan.py` | 从 exe 里挖类/资源/控件名 | 387 个 metaObject、77 个图标 |

## 各部分成色

**逐字还原、可复跑验证**

- 43 个应用类的类名、继承链、全部 signals/slots/properties/enums
  （`re/moc_scan.py`：387 个 staticMetaObject 全部解码成功）
- 77 个 qrc 图标，原始字节，资源路径与原程序一致（`:/icon/icons/*.png`）
- 6 个对话框的控件构成，由 QStringLiteral 与 `on_<控件名>_clicked` 自动连接槽双向印证
...
| `GlobalSettings` | 编辑器偏好，存 QSettings("QtProject","UITools") |
...
- 事件动作（`element_event_action`）的**内容**：二进制布局已知（固件
  `struct event_action`），编辑器和生成器都照着写了，但本工程 277 个控件的
  action 全是 `num=0`，**没有真实样本可对拍**。
- 图片的 RLE / QuickLZ 压缩：本工程 `image_compress_method = none`，
  没有样本。固件里解码器还在（`liba/res/rle.c`、`quicklz.c`），要用再反着写。
- 调色板前缀的排列顺序（集合已复现）。
...
`ename.h` 里每个宏的值是 24 位 ID，位域已确定：

```
bit23..22 页索引    bit21..16 控件类型(== .sty 控件头 type)    bit15..0 名字哈希
```

低 16 位那个哈希没解出来。已排除（281 条样本全部 0 命中）：djb2 / sdbm /
BKDR(31,131,1313) / FNV-1a / AP 各种掩码；CRC16 的 9 种标准变体；
**CRC16 全 65536 个多项式 × 3 种 init × 2 种移位方向 × 6 种输入变体**；
乘法哈希 mul 遍历 2..4095 × 5 种 init；输入换成中文 `-name`（UTF-8/GBK）、
`caption`、`-type`。也确认了 ID 不存在任何中间文件里。
...
所以重建版用自己的确定性方案：**`CRC16-XMODEM(宏名)`，撞了就线性探测**。
产出的 `.sty` + `ename.h` 完全可用。只有要和原厂**逐字节对拍**时才需要
`QtToolBin --ename <原厂 ename.h>` 把原来的 id 借过来。

顺带一提，最合理的解释是原厂用了 **Qt 带随机种子的 `qHash`**，或者
"随机分配 + 查重"——两者都意味着原厂同一份工程每次生成的 id 都不一样。
