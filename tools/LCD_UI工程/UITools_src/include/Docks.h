 ...
 * Docks.h —— 主窗口四周的停靠面板
 *
 * 【整体布局】加载 SmallColorTFT.json 之后的形态：
 *
 *   ┌────────────────────────────────────────────────────────────────────┐
...
/** "CSS属性_N" 页签容器。一个 CSS 状态一个页签，当前实现只有 _0。 */
/**
 * 属性区的两个页签：「基础设置」和「资源」。
 *
 * 【为什么拆两页】上下堆叠一整列的话（ID号 / CSS属性 / 控件专有属性区），
 * 一个「文字」控件铺满 22 行，窄边栏里必须滚。拆成两页之后最高的「时间」
 * 也只有 13 行。原本"一个 CSS 状态一个页签"那层降级成常驻的下拉框 ——
...
public slots:
    /* CSS 状态的增删。手册 2.10 说的是"右键点击菜单项的 CSS 属性_0，
     * 选择复制添加"；这里页签是另一个含义，所以这套菜单跟着挪到
     * **状态下拉框**的右键上，动作不变。 */
