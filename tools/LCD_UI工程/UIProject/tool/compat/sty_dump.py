# -*- coding: utf-8 -*-
"""解析 JL.sty(= project.bin)，验证对 .sty 容器格式的理解。

结构依据（三个来源互相印证，不是猜的）：
  1) 固件侧解析器
       struct ui_file_head / struct ui_ctrl_info_head
  2) 与之配套的结构转储  .../project/debug.txt
...
def main():
    ...
    print('\n== window_head 表（与 debug.txt 的 offset/prop_len/t_ptr/t_size/crc 逐列比对）==')
