# -*- coding: utf-8 -*-
"""验证：控件类型码完全来自工具目录 assets/typecodes.ini 的 [Control] 段。

做法：
  ename.h 里每个宏的 id -> (id>>16)&0x3F 得到实际写进 .sty 的 type；
  工程 json 里同名 ename 的节点 -> 取它的 "-type"；
  用 option.ini 把 "-type" 映射成类型码，两边比对。
...
def main():
    ...
    for ename, (pi, jtype, jcls, jcap) in info.items():
        ...
        got_type = (v >> 16) & 0x3F
        got_page = (v >> 22) & 0x7F
        # 扩展控件（自定义控件目录里的 slider / vslider 及其零件）的 -type 都是
    ...
    print('\n类型码比对: 一致 %d / 不一致 %d / 无法对照 %d' % (ok, bad, miss))
    for e, t, w, g in badlist:
        print('   %-24s -type=%-14s option.ini=%-3s 实际=%s' % (e, t, w, g))
    print('页号比对: 不一致 %d / 共 %d' % (pagebad, ok + bad))
