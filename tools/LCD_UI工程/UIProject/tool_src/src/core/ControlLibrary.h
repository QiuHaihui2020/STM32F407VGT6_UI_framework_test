 ...
 *
 * 【好消息】这一块不在程序里，是外部数据文件（位置见 AssetPaths.h）：
 *   <工具目录>/assets/widgets.json      10 个内置控件模板
 *   <工具目录>/assets/widgets.d/*.json  扩展控件（slider / vslider）
...
struct ControlTemplate {
    ...
    QString     iconPath;   ///< 相对工具目录
    QIcon       icon;
    bool        isExtension = false;  ///< 来自自定义控件目录 —— 单独放在"自定义控件"组
