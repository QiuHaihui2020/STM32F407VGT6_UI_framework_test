...
//
// 权威来源是固件自己的解析器：
//   头 / 页表 / 条目表 / 取图；单色像素的读法见下面各字段的说明
//
// 所有验证脚本在 compat/ 下，实测结论见 docs/FILE_FORMATS.md 第 10 节。
#ifndef RESFORMAT_H
#define RESFORMAT_H
...
struct Head {
    ...
    quint16  panelType;     // LCDPANEL=0 / OLEDPANEL=1（对应 paneltype 字段）
...
struct BmpEntry {
    ...
    quint16 data_crc;       // CRC16-XMODEM(像素数据)；.res 里恒写 0
...
// typeId 里的 format 字段。点阵屏工程用的是 OSD1（1bpp 单色），编码为 0。
 ...
 * @note  这是**真实**长度。写进 dwLength 的是 (w+7)*pages ——
...
/**
 * @brief 写进 dwLength 的值（保留这个 bug，好和既有资源逐字节对齐）
 * @note  括号打错的位置是 **h 那一边**：正确写法 ((w+7)/8)*h，实际写成
 *        (w+7)*h/8。当 h 是 8 的倍数时两种理解（(w+7)*(h/8) 与 (w+7)*h/8）
 *        结果相同，全工程 314 张图里只有 26x26 那 4 张 h 不是 8 的倍数，
 *        既有资源里是 (26+7)*26/8 = 107 而不是 (26+7)*4 = 132 —— 以此定案。
