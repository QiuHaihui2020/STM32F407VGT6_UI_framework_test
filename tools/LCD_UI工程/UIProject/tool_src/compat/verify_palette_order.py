...
"""把「调色板差异」这件事钉住：集合必须相同，差异必须只在顺序上。
    ...
    python verify_palette_order.py <本次产物目录> <参考目录>
...
背景（详见 docs/QTTOOLBIN.md「ColorList 排序」一节）：
  ...
  按 Resbuilder.xml 的 ColorList 原序写，不重排。两边的 ColorList **集合相同、
  顺序不同**，参考文件那个顺序试遍了各种假设都找不到规律（很可能是 Qt 容器
  + 随机 hash seed，重跑一次也会变）。
...
RESVER_RANGE = (0x0C, 0x10)   # 4 字节 resver，参考就是随机值
...
def colors_of(xml_path):
    """按页取 ColorList。xml 是 UTF-8（参考如此），历史上本次写过 GBK，两种都认。"""
...
def main():
    ...
    if len(mine_c) != len(ref_c):
        print('★ 页数不同：本次 %d / 参考 %d' % (len(mine_c), len(ref_c)))
        bad += 1
    for i, (a, b) in enumerate(zip(mine_c, ref_c)):
        ...
        if sa == sb and len(a) == len(b):
            ...
            print('    只在本次: %s' % ' '.join(sorted(sa - sb)))
            print('    只在参考: %s' % ' '.join(sorted(sb - sa)))
            bad += 1
    ...
    for i, cs in enumerate(ref_c):
        ...
        if got is None:
            print('★ 页%d 的调色板在参考 result.bin 里找不到（格式假设不成立？）' % i)
            bad += 1
        else:
            spans.append(got)
