...
（2026-09-09 在同一份工程上连跑三遍逐项比对确认，
详见 docs/FILE_FORMATS.md 10.10 / 10.11）：

  project.bin  Time(+28) / number(+24) 的 char format[16]，参考文件只 strcpy 了
               strlen+1 个字节，剩下十来个字节是没清的堆内存（里面躺着两个
               活指针）。重跑一遍那些指针也全变。这里整片清零。
               外加没有子控件的 NewLayout：参考文件在子指针里留了兄弟指针的值，
               本次写 0（固件 layer.c 先判 ctrl_num 再解引用，写 0 是安全的）。
  result.bin   4 字节 resver（随机值）
               + 调色板的**排列顺序**：<ColorList> 的次序来自 Qt5 QSet
                 迭代，随机哈希种子，跑三遍三个样。集合一致，OSD1 单色屏也
                 根本不查调色板。本次用确定性排序，同工程跑多少遍都一样。
  result.str   4 字节 resver
  result.xml   同样只差 <Color> 的排序
  result_str_index.h  时间戳 + 少数几行的**位置**（<CellList> 也是随机序）
...
这里说的"参考"，指的是工程目录里现成的那几个文件（project.bin / result.* …）。
它们是当初生成后留在那儿的，**不受 git 管**（.gitignore 里有 *.bin），
而任何一次 step2 都会就地把它们重写一遍。
...
就是被覆盖的。要恢复真正的参考文件，只能用当初那套工具
对着同一个工程再跑一次 step2（本次工具生成的不能当参考用 —— 拿自己比自己，
比过了也说明不了什么）。
"""
import datetime
import io
...
def explain_text(name, a, b):
    """文本产物按**行**比。返回不可解释的行（行号, 本次, 参考）。

    只允许两类整行不同：
      · 行里有时间戳（生成时间，每次都变）
      · result.xml 的 <Color .../> —— <ColorList> 的次序来自 Qt5 QSet
...
def explain_sty(a, b):
    """project.bin 只允许三种差异，别的都算不可解释：
      ...
      · 本次写 0x00、参考是垃圾的连续段 —— Time(+28)/number(+24) 的
        char format[16]，参考文件只 strcpy 了 strlen+1 个字节，剩下是没清的堆内存
        （里面躺着两个活指针，重跑一遍也全变）；这里整片清零。
    """
...
def explain_res(a, b):
    """result.bin / result.str 允许两种差异，别的都算不可解释：
      ...
      · [0x0C,0x10)  resver —— 随机值，跑三遍三个样
      · **调色板次序** —— <ColorList> 的次序来自 Qt5 QSet 的随机哈希
...
def main():
    ...
    # 工程后缀两种都认：本工具链产出的 .uiproj，既有工程的 .json。
    jsons = [f for f in os.listdir(proj)
             if f.lower().endswith(('.uiproj', '.json'))
             and 'autosave' not in f.lower()]
    if not jsons:
        print('工程目录里没有工程文件（*.uiproj / *.json）')
        return 2
    ...
    print('工程: %s' % pick)
    # 多国语言表：按后缀在几个候选目录里找，不写死文件名和目录名。
    # 工程在工具目录下面时是 ../../../assets/；工具目录和工程并列的老布局
    # 也照顾到。写死一个名字的话，换块屏或者目录改个名这里就静默传错路径。
    xls = ''
    for cand in (('..', '..', '..', 'assets'),
                 ('..', '..', '..'),
                 ('..', '..', '..', 'UIToolkit', 'assets'),
                 ('..', '..', '..', 'UIToolkit'),
                 ('..', '..', '..', 'UITools')):
        d = os.path.abspath(os.path.join(proj, *cand))
        if not os.path.isdir(d):
            continue
        hit = sorted(f for f in os.listdir(d) if f.lower().endswith('.xls'))
        if hit:
            xls = os.path.join(d, hit[0])
            break
    if not xls:
        print('找不到多国语言 xls（在工程上三级的 assets/ 里找）')
        return 2
    ...
    #   result.bin   被另一个工程的产物顶成 11451（真值 10953），而旧的
    #                "差异字节数 <= 预算"判定还一路报"允许范围内" —— 假绿
    # 所以在 compat/refs/ 存了一份基准，跑之前对一下就还原。
    ...
    for prod in ('project.bin', 'result.bin'):
        ...
        if cur != want:
            ...
            print('（%s 参考被顶过，已从 compat/refs 存档还原：%d -> %d 字节）'
    ...
    print('%-24s %-8s %-8s %-8s %s' % ('文件', '本次', '参考', '差字节', '判定'))
    bad = 0
    for n in FILES:
        ...
        if n == 'project.bin':
            for s0, p, xa, xb in explain_sty(a, b):
                unexplained.append('@0x%06X..0x%06X 本次=%s 参考=%s' % (s0, p, xa, xb))
        elif n in ('result.bin', 'result.str'):
            for s0, p, xa, xb in explain_res(a, b):
                unexplained.append('@0x%06X..0x%06X 本次=%s 参考=%s' % (s0, p, xa, xb))
        elif len(a) != len(b):
            unexplained.append('长度就不一样：本次 %d / 参考 %d' % (len(a), len(b)))
        else:
            for ln, xa, xb in explain_text(n, a, b):
                unexplained.append('第 %d 行  本次=%s  参考=%s' % (ln, xa, xb))
